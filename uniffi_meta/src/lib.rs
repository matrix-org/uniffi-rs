...
use syn::{
    FnArg, ImplItemMethod, ItemEnum, ItemFn, ItemImpl, ItemStruct, Pat, ReturnType, Type, Variant,
};
...
impl FnParamMetadata {
    pub fn new(a: &FnArg, _is_method: bool) -> syn::Result<Self> {
        let (name, ty) = match a {
            FnArg::Receiver(_) => unimplemented!(),
            FnArg::Typed(pat_ty) => {
                let name = match &*pat_ty.pat {
                    Pat::Ident(pat_id) => pat_id.ident.to_string(),
                    _ => unimplemented!(),
                };
                (name, &pat_ty.ty)
            }
