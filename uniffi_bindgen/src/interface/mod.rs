...
use std::{collections::HashSet, convert::TryFrom, iter, str::FromStr};
...
impl ComponentInterface {
    ...
    pub fn ffi_namespace(&self) -> String {
        format!("__uniffi_{}", self.namespace)
    }
    ...
    pub fn ffi_rustbuffer_alloc(&self) -> FFIFunction {
        FFIFunction {
            name: "uniffi_rustbuffer_alloc".to_owned(),
    ...
    pub fn ffi_rustbuffer_from_bytes(&self) -> FFIFunction {
        FFIFunction {
            name: "uniffi_rustbuffer_from_bytes".to_owned(),
            arguments: vec![FFIArgument {
    ...
    pub fn ffi_rustbuffer_free(&self) -> FFIFunction {
        FFIFunction {
            name: "uniffi_rustbuffer_free".to_owned(),
            arguments: vec![FFIArgument {
                name: "buf".to_string(),
    ...
    pub fn ffi_rustbuffer_reserve(&self) -> FFIFunction {
        FFIFunction {
            name: "uniffi_rustbuffer_reserve".to_owned(),
            arguments: vec![
                FFIArgument {
                    name: "buf".to_string(),
                    type_: FFIType::RustBuffer,
    ...
    pub(super) fn add_function_definition(&mut self, defn: Function) -> Result<()> {
        // FIXME(jplatte): Should add function args to types as a known type
        if let Some(ty) = defn.return_type.clone() {
    ...
    pub fn derive_ffi_funcs(&mut self) -> Result<()> {
        let ffi_ns = self.ffi_namespace();
        for func in self.functions.iter_mut() {
            func.derive_ffi_func(&ffi_ns)?;
        }
        for obj in self.objects.iter_mut() {
            obj.derive_ffi_funcs(&ffi_ns)?;
        }
        for callback in self.callback_interfaces.iter_mut() {
            callback.derive_ffi_funcs(&ffi_ns);
        }
        Ok(())
    }
}
...
mod test {
    ...
    fn test_contains_optional_types() {
        let mut ci = ComponentInterface::new("foo".to_owned());
