...
a `#[uniffi::export]`ed function or method. Used on its own, it can be used on any enum that the
`Enum` derive also accepts, and exposes any variant fields to the foreign code. The generated
foreign function will have the result's `T` as its return type, and throw the error in case the Rust
call returns `Err(e)`.
...
You can also use the helper attribute `#[uniffi(flat_error)]` to expose just the variants but none
of the fields, in this case the error will be serialized using Rust's `ToString` trait and will be
accessible as the only field on each of the variants. In this case, your variants can have unnamed
fields, and the types of the fields don't need to implement any special traits.
